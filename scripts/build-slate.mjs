#!/usr/bin/env node
/**
 * Rebuild slate.json from ESPN's public college-football feed.
 *
 *   node scripts/build-slate.mjs --probe     inspect the feed, write nothing
 *   node scripts/build-slate.mjs             update slate.json
 ...
 *   2. Adds the new week's games — every game with an AP Top 25 team, plus
 *      Florida's game — and points currentWeek at it.
 *
...
const FBS = 80;                 // groups=80 restricts to FBS
const ALWAYS = ["FLA"];         // always included regardless of ranking
const UNRANKED = 99;            // ESPN uses 99 for "not ranked"
...
async function getJSON(url) {
  const r = await fetch(url, { headers: { "User-Agent": "cfb-pickem/1.0" } });
  if (!r.ok) throw new Error(`${r.status} ${r.statusText} for ${url}`);
  return r.json();
}

const scoreboard = (params) =>
  getJSON(`${API}/scoreboard?groups=${FBS}&limit=400&${params}`);
...
async function probe() {
  log("=== PROBE: ESPN college-football feed ===\n");
...
function qualifies(comp) {
  ...
  if (cs.some((c) => rankOf(c) !== null)) return true;
...
async function resultsForWeek(weekNo, gameIds) {
  ...
  for (const id of gameIds) {
    ...
    if (!e) { warn(`game ${id} not found in week ${weekNo} feed`); continue; }
    const comp = e.competitions?.[0];
    if (!e.status?.type?.completed) { unfinished++; continue; }
...
async function main() {
  ...
  if (prev && slate.currentWeek !== espnWeek) {
    const ids = (prev.games ?? []).map((g) => g.id);
    const { results, unfinished } = await resultsForWeek(slate.currentWeek, ids);
    if (unfinished) {
  ...
  for (const g of games) {
    if (g.awayRank) ranked.add(g.awayRank);
    if (g.homeRank) ranked.add(g.homeRank);
  }
  const byes = [];
  for (let r = 1; r <= 25; r++) if (!ranked.has(r)) byes.push(r);
  if (byes.length) log(`Top 25 not playing a qualifying game (bye or FCS): #${byes.join(", #")}`);

  /* Refuse to overwrite a week that has already been graded with a different
     set of games. ESPN's week numbering has been observed disagreeing with
     itself, and a collision here would orphan every pick made that week —
     the picks reference game ids that would no longer exist in the slate. */
  const existing = slate.weeks[String(espnWeek)] ?? {};
  const oldIds = (existing.games ?? []).map((g) => String(g.id)).sort().join(",");
  const newIds = games.map((g) => String(g.id)).sort().join(",");
  if (oldIds && oldIds !== newIds && existing.results && Object.keys(existing.results).length) {
    throw new Error(
      `week ${espnWeek} already holds ${existing.games.length} graded game(s) that ` +
      `differ from the ${games.length} ESPN now reports for that week. Refusing to ` +
      `overwrite and orphan those picks. Re-key the stored week in slate.json, or ` +
      `run with --week to place these games elsewhere.`
    );
  }
  if (oldIds && oldIds !== newIds) {
    warn(`replacing week ${espnWeek}'s ungraded games (${existing.games.length} -> ${games.length})`);
  }
